...
public class AI_Skelepile : MonoBehaviour {
	...
	void Update () {
		...
		if(AttackDelay)
		{
			...
			if(BigAttackDelay > 0.333f)
			{
				gameObject.transform.position = new Vector2(ShakePos.x + Random.Range(BigAttackDelay-2,-BigAttackDelay+2) * 0.33f,ShakePos.y + Random.Range(BigAttackDelay-1.222222f,-BigAttackDelay+2) * 0.33f);
			}

			if(BigAttackDelay > 1.5f)
			{
				ActuallyAttack();
			}
		}
		...
		if(AttackStopSwoosh)
		{
			...
			Fist.gameObject.transform.position = new Vector3(DeathCross.transform.position.x,DeathCross.transform.position.y + 32 * (0.2f - BigAttackDelay) + 2,-5);

			if(BigAttackDelay > 0.2f)
			{

				GameObject badsplosion =	Instantiate(Explosion,DeathCross.transform.position,gameObject.transform.rotation) as GameObject;
				badsplosion.name = "SheepHurtbox";

				Fist.gameObject.transform.localPosition = new Vector3(0,0,-500);
				DeathCross.gameObject.transform.localPosition = new Vector3(0,0,-500);
	...
	void Attack()
	{
		...
		DeathCross.transform.localPosition = new Vector3(Dir.x*5,Dir.y *5,1);
		Fist.gameObject.transform.position = new Vector3(DeathCross.transform.position.x,DeathCross.transform.position.y + 8,-5);
