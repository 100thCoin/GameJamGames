	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float2 Ripples = sin(modUV.y*_RippleScaleV + _Time.y*_RippleSpeed+_RippleOffset)*(_RippleScale+(modUV.y-0.24)*0.02);
				...
				float4 col = tex2D(_MainTex, modUV + float2(Ripples.x*_Force,0));
